...
object Server extends App {
  def handle(req: Request, table: String,
    f: String => Future[JValue]): Future[Response] = {
    for {
  ...
  lazy val route = Service.mk[Request, Response] { req =>
    (req.method, Path(req.path)) match {
      ...
      case (Get | Options, Root / (table: String)) => handle(req, table,
        MySQL.read)
        ...
        case e: ServerError if e.code == 1062 || e.code == 1169 =>
          CANNED(Conflict,
            ("code" -> e.code) ~ ("message" -> e.message) ~
              ("details" -> e.sqlState) ~ ("hint" -> null))

        // Other SQL errors
        case e: ServerError =>
          CANNED(InternalServerError,
            ("code" -> e.code) ~ ("message" -> e.message) ~
              ("details" -> e.sqlState) ~ ("hint" -> null))
